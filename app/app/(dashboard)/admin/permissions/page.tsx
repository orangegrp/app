...
import { isSuperuserPermissionsCsv, PERMISSIONS, SUPERUSER_PERMISSION } from '@/lib/permissions'
...
function selectionFromPermissionsCsv(csv: string): { superuser: boolean; selected: Set<string> } {
...
export default function AdminPermissionsPage(): ReactNode {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiGet<ListResponse>(`/admin/users?${params.toString()}`)
      setUsers(data.users)
      setTotal(data.total)
    } catch {
  ...
  const openEdit = (u: AdminUserRow): void => {
    setEditing(u)
    setSaveError(null)
    const { superuser: su, selected: sel } = selectionFromPermissionsCsv(u.permissions)
    setSuperuser(su)
    setSelected(sel)
  ...
  const handleSave = async (): Promise<void> => {
    ...
    setSaveLoading(true)
    setSaveError(null)
    const displayName = editDisplayName.trim() === '' ? null : editDisplayName.trim()
    try {
      const body =
        editingSelf
          ? { displayName }
      ...
      const res = await apiPatch<{ user: AdminUserRow }>(`/admin/users/${editing.id}`, body)
  ...
  return (
    <div className="page-root relative min-h-screen px-6 pb-32 pt-8 sm:pt-10">
      ...
      <div className="relative z-10 mx-auto max-w-3xl">
        ...
        <p className="text-xs text-muted-foreground tracking-wider mb-8 max-w-xl">
          Assign permissions from the canonical set. Changes apply after the user&apos;s next token refresh.
        </p>

        <div className="glass-card rounded-2xl p-4 mb-6 flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="flex-1 flex flex-col gap-1.5">
            <label className="text-xs tracking-wider text-muted-foreground" htmlFor="user-search">
            ...
            <input
              ...
              placeholder="Filter…"
              className="glass-card rounded-xl px-4 py-3 text-sm tracking-wider text-foreground placeholder:text-muted-foreground/40 bg-transparent border-0 outline-none focus:ring-1 focus:ring-white/10 w-full min-h-[44px]"
            />
          </div>
        </div>

        {listError && (
          <p className="mb-4 text-xs tracking-wider" style={{ color: 'oklch(0.7 0.19 22)' }}>
            {listError}
          </p>
        )}
        ...
        {loading ? (
          ...
          <>
            <div className="glass-card rounded-2xl overflow-hidden divide-y divide-white/5">
              ...
              {users.length === 0 ? (
                <div className="px-4 py-8 text-xs tracking-wider text-muted-foreground text-center">
                  No users match.
                </div>
              ) : (
                users.map((u) => (
                  <div
                    key={u.id}
                    className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-2 px-4 py-3 items-start sm:items-center"
                  >
                    <div>
                      <p className="text-sm tracking-wider text-foreground flex flex-wrap items-center gap-2">
                        <span>
                      ...
                      {u.displayName?.trim() && u.discordUsername && (
                        <p className="text-[11px] tracking-wider text-muted-foreground/80 mt-0.5 truncate" title={u.discordUsername}>
                          Discord: {u.discordUsername}
                        </p>
                      )}
                      <p className="font-mono text-[11px] tracking-wider text-muted-foreground/80 mt-0.5">
                        {shortId(u.id)}
                      </p>
                      <p
                        className="sm:hidden text-[11px] tracking-wider text-muted-foreground mt-1 break-all"
                        title={u.permissions}
                      >
                    ...
                    <p
                      className="hidden sm:block text-xs tracking-wider text-muted-foreground truncate max-w-[220px]"
                      title={u.permissions}
                    >
                      {permissionsPreview(u.permissions)}
                    </p>
                    <div className="flex justify-end">
                      <button
                        type="button"
                        onClick={() => openEdit(u)}
                        className="glass-button glass-button-ghost rounded-lg px-3 py-2 text-xs tracking-widest inline-flex items-center gap-2 min-h-[44px] min-w-[44px]"
                      >
                        <Pencil size={14} />
                        Edit
                      </button>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-6 text-xs tracking-wider text-muted-foreground">
                <span>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="glass-button glass-button-secondary rounded-lg px-3 py-2 disabled:opacity-40 min-h-[44px]"
                  >
                    Previous
                  </button>
                  <button
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto" showCloseButton>
          <DialogHeader>
            ...
            <DialogDescription>
              {editing?.displayName?.trim() || editing?.discordUsername || shortId(editing?.id ?? '')}
            </DialogDescription>
          </DialogHeader>

          <div className="flex flex-col gap-4 py-2">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="admin-edit-display-name" className="text-xs tracking-widest text-muted-foreground">
                Display name
              </Label>
              <input
              ...
              <p className="text-[11px] text-muted-foreground tracking-wider">
                Does not change Discord; max {DISPLAY_NAME_MAX_LENGTH} characters.
              </p>
            </div>

            <div
              className={cn(
                'rounded-xl border border-white/10 px-3 py-3 space-y-2',
                (editingSelf || cannotEditTargetSuperuserPermissions) &&
                  'opacity-60 pointer-events-none',
              )}
            >
              <p className="text-xs tracking-widest text-muted-foreground uppercase">Roles and access</p>
              {editingSelf && (
                <p className="text-[11px] text-muted-foreground tracking-wider">
                  You cannot change your own permissions or superuser status here.
                </p>
              )}
              {cannotEditTargetSuperuserPermissions && (
                <p className="text-[11px] text-muted-foreground tracking-wider">
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="space-y-0.5 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Label htmlFor="superuser" className="text-sm tracking-wider">
                      Superuser
                    </Label>
              ...
              <div
                className={cn(
                  'space-y-3 pt-1',
                  superuser && !editingSelf && !cannotEditTargetSuperuserPermissions && 'opacity-40 pointer-events-none',
                )}
              >
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <Checkbox
                      ...
                      disabled={superuser || editingSelf || cannotEditTargetSuperuserPermissions}
                    />
                    <Label
                      htmlFor={permFieldId}
                      className="text-sm tracking-wider font-normal cursor-pointer"
                    >
                      {label}
                    </Label>
                  </div>
                  <code
                    className="text-[11px] font-mono text-muted-foreground shrink-0 max-w-[50%] truncate text-right"
                    title={perm}
                  >
                    {perm}
                  </code>
            ...
            >
              <p className="text-xs tracking-widest text-muted-foreground uppercase">Security</p>
              <div className="flex items-center justify-between gap-4">
                <div className="space-y-0.5 min-w-0">
                  <Label htmlFor="admin-login-allowed" className="text-sm tracking-wider">
                    Login allowed
                  </Label>
                  <p className="text-xs text-muted-foreground tracking-wider">
                    Turn off to security-freeze the account (blocks sign-in and refresh).
                  </p>
                </div>
                <Switch
            ...
            {!editingSelf && (
              <div className="rounded-xl border border-red-500/25 px-3 py-3 space-y-2">
                <p className="text-xs tracking-widest text-muted-foreground uppercase">Danger zone</p>
                <p className="text-xs text-muted-foreground tracking-wider">
                  Permanently delete this user and revoke access. Invite codes they created remain; sessions end immediately.
                </p>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="glass-button glass-button-destructive rounded-lg px-4 py-2.5 text-xs tracking-widest min-h-[44px]"
                >
                  Delete user
                </button>
              </div>
          ...
          <DialogFooter>
            ...
            <Button type="button" disabled={saveLoading} onClick={() => void handleSave()}>
      ...
      >
        <AlertDialogContent className="sm:max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle className="tracking-widest">Delete user?</AlertDialogTitle>
            <AlertDialogDescription>
              This cannot be undone. Type the user&apos;s full account ID to confirm.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="grid gap-3 py-2">
            <div>
              <label className="text-xs tracking-wider text-muted-foreground block mb-1.5">
                Account ID
              </label>
              <Input
                ...
                autoComplete="off"
                aria-invalid={deleteConfirmId.length > 0 && deleteConfirmId.trim() !== editing?.id}
              />
            </div>
            {deleteError && (
              <p className="text-xs tracking-wider" style={{ color: 'oklch(0.7 0.19 22)' }}>
