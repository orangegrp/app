...
export default function SettingsPage() {
  ...
  const [passkeyToRemove, setPasskeyToRemove] = useState<PasskeyRow | null>(null)
  ...
  useEffect(() => {
    ...
    const map: Record<string, string> = {
      ...
      already_linked: 'This account already has a different Discord linked — disconnect it first.',
  ...
  const saveDisplayName = async () => {
    ...
    try {
      const next = await apiPatch<MeProfile>('/me/profile', {
        displayName: displayNameDraft.trim() === '' ? null : displayNameDraft.trim(),
      })
      setMe(next)
      useAuthStore.getState().mergeAuthUser({ displayName: next.displayName ?? null })
    } catch (e) {
      setDisplayNameError(e instanceof Error ? e.message : 'Could not save display name')
    } finally {
  ...
  const patchLoginMethods = async (
    partial: Partial<
      Pick<MeProfile, 'loginPasskeyEnabled' | 'loginDiscordEnabled' | 'loginMagicEnabled' | 'loginQrEnabled'>
    >
  ) => {
    ...
    try {
      ...
      setMethodsError(e instanceof Error ? e.message : 'Could not update sign-in methods')
    } finally {
  ...
  const handleConnectDiscord = async () => {
    ...
    try {
      const { url } = await apiPost<{ url: string }>('/auth/discord/link-start', {
        isPwa: isPWAContext(),
      })
      hardNavigateTo(url)
    } catch {
  ...
  const handleUnlinkDiscord = async () => {
    try {
      ...
      setDiscordNotice(e instanceof Error ? e.message : 'Could not disconnect Discord')
    }
  }
  ...
  const handleAddPasskey = async () => {
    ...
    setPkError(null)
    try {
      const { options } = await apiPost<{ options: unknown }>('/auth/add/start', {})
  ...
  const handleRemovePasskey = async () => {
    ...
    try {
      await apiFetch(`/auth/passkeys/${passkeyToRemove.id}`, { method: 'DELETE' })
  ...
  const handleConfirmDeleteAccount = async () => {
    ...
    try {
      ...
      setDeleteError('Could not delete account. Check the fields and try again.')
    } finally {
      setDeleting(false)
    }
  }
    ...
    <div className="page-root relative min-h-screen px-6 pb-32 pt-8 sm:pt-10">
      ...
      <div className="relative z-10 mx-auto max-w-2xl">
        ...
        <div className="glass-card rounded-2xl p-6 mb-6">
          ...
          <p className="text-sm tracking-wider text-foreground mb-4">{displayName}</p>
          <div className="flex flex-col gap-1.5">
            <label htmlFor="settings-display-name" className="text-xs tracking-wider text-muted-foreground">
              Display name
            </label>
            ...
            <p className="text-[11px] text-muted-foreground tracking-wider">
              Separate from Discord. Max {DISPLAY_NAME_MAX_LENGTH} characters. Clear to use Discord or ID.
            </p>
          </div>
          {displayNameError && (
            <p className="mt-2 text-xs tracking-wider" style={{ color: 'oklch(0.7 0.19 22)' }}>
              {displayNameError}
            </p>
          )}
          {user?.id && (
            <p className="mt-4 text-xs font-mono tracking-wider text-muted-foreground/80 break-all">
              ID: {user.id}
            </p>
          )}
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 mb-6">
          ...
          {discordNotice && (
            <p className="text-xs tracking-wider text-muted-foreground mb-3">{discordNotice}</p>
          )}
          {meLoading ? (
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] tracking-wider text-muted-foreground/80">Linked for sign-in and profile</p>
                </div>
              </div>
              <button
                type="button"
                ...
                className="glass-button glass-button-secondary rounded-xl px-4 py-2.5 text-xs tracking-widest shrink-0 disabled:opacity-40"
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                ...
                className="glass-button glass-button-glass rounded-xl px-4 py-2.5 text-xs tracking-widest inline-flex items-center justify-center gap-2 shrink-0"
                style={{ minHeight: '44px' }}
              >
        ...
        <div className="glass-card rounded-2xl p-6 mb-6">
          ...
          <p className="text-xs text-muted-foreground tracking-wider mb-4">
            Turn off methods you do not want to allow (like freezing a card). At least one must stay on.
          </p>
          {methodsError && (
            <p className="text-xs tracking-wider mb-3" style={{ color: 'oklch(0.7 0.19 22)' }}>
              {methodsError}
            </p>
          )}
          {meLoading || !me ? (
            ...
            <div className="divide-y divide-white/5 rounded-xl border border-white/5 overflow-hidden">
              {(
                ...
                <div key={key} className="flex items-center justify-between gap-4 px-4 py-3">
                  <div className="min-w-0">
                    <p className="text-sm tracking-wider text-foreground">{label}</p>
                    <p className="text-[11px] text-muted-foreground tracking-wider">{desc}</p>
                  </div>
                  <Switch
                    checked={me[key]}
                    onCheckedChange={(v) => void patchLoginMethods({ [key]: v })}
        ...
        <div className="glass-card rounded-2xl p-6 mb-6">
          ...
          {pkLoading ? (
            ...
            <>
              {passkeys.length === 0 ? (
                <p className="text-xs tracking-wider text-muted-foreground/70 py-2">No passkeys yet.</p>
              ) : (
                <ul className="divide-y divide-white/5 mb-4 rounded-xl overflow-hidden border border-white/5">
                  {passkeys.map((pk) => (
                    <li key={pk.id} className="flex items-center gap-3 px-4 py-3">
                      <Key size={16} className="shrink-0 text-muted-foreground" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm tracking-wider truncate">
                      ...
                      <button
                        ...
                        className="glass-button glass-button-ghost rounded-lg p-2 text-muted-foreground hover:text-foreground shrink-0"
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-3">
                ...
                <button
                  ...
                  disabled={addingPk}
                  className="glass-button glass-button-glass h-9 shrink-0 rounded-lg px-5 text-sm tracking-widest disabled:opacity-50 inline-flex items-center justify-center"
                >
                  {addingPk ? <Spinner size="xs" /> : 'Add passkey'}
                </button>
              </div>
              {pkError && (
                <p className="mt-3 text-xs tracking-wider" style={{ color: 'oklch(0.7 0.19 22)' }}>
                  {pkError}
                </p>
              )}
            </>
        ...
        >
          <p className="card-label mb-2" style={{ color: 'oklch(0.75 0.12 22)' }}>
            Danger zone
          </p>
          <p className="text-xs text-muted-foreground tracking-wider mb-4">
      ...
      <AlertDialog open={unlinkDiscordOpen} onOpenChange={setUnlinkDiscordOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You can reconnect later. You need at least one passkey so you can still sign in without Discord.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel type="button">Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => void handleUnlinkDiscord()}>Disconnect</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

      <AlertDialog open={!!passkeyToRemove} onOpenChange={(open) => !open && setPasskeyToRemove(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove passkey?</AlertDialogTitle>
            <AlertDialogDescription>
              You will no longer be able to sign in with this device until you add a new passkey.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => void handleRemovePasskey()}>Remove</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      >
        <AlertDialogContent className="sm:max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently removes your account and passkeys. You will need a new invite to register again.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="grid gap-3 py-2">
            <div>
              ...
              <Input
                ...
                autoComplete="off"
                aria-invalid={deleteDialogPhrase.length > 0 && deleteDialogPhrase !== 'delete my account'}
              />
            </div>
            <div>
              <label className="text-xs tracking-wider text-muted-foreground block mb-1.5">
