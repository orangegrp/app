...
vi.mock('@/server/db', () => ({
  db: {
    validateAndMigrateSchema: (...args: unknown[]) => mockValidateAndMigrateSchema(...args),
    cleanupExpiredRecords: (...args: unknown[]) => mockCleanupExpiredRecords(...args),
  },
}))
...
describe('GET /api/cron/cleanup', () => {
  ...
  beforeEach(() => {
    ...
    process.env.SUPABASE_SERVICE_ROLE_KEY = 'test-service-role-key-for-vitest-only'
  ...
  it('returns 401 when Authorization is missing', async () => {
    const app = await loadApp()
    const res = await app.request('http://test.local/api/cron/cleanup', { method: 'GET' })
