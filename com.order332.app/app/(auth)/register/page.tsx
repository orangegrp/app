...
export default function RegisterPage() {
  ...
  const handleClaimInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status >= 500) {
          setErrorMsg(data.error ?? 'Something went wrong on our side. Please try again in a moment.')
        } else {
          setErrorMsg(data.error ?? 'Invalid invite code')
        }
        return
      }
      const { registrationToken: token } = await res.json() as { registrationToken: string }
  ...
  const handlePasskeyRegister = async () => {
    ...
    try {
      ...
      const { accessToken } = await finishRes.json() as { accessToken: string }
  ...
  return (
    <section className="page-root relative flex min-h-screen items-center justify-center px-4">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-clip" aria-hidden="true" style={{ contain: 'layout style paint' }}>
        <div className="absolute left-[15%] top-[20%] h-[500px] w-[500px] rounded-full"
          style={{ background: 'radial-gradient(circle, oklch(0.9 0 0 / 5%), transparent 65%)', filter: 'blur(80px)', animation: 'float-orb-1 28s ease-in-out infinite' }} />
        <div className="absolute right-[10%] top-[40%] h-[400px] w-[400px] rounded-full"
          style={{ background: 'radial-gradient(circle, oklch(0.8 0 0 / 4%), transparent 65%)', filter: 'blur(60px)', animation: 'float-orb-2 34s ease-in-out infinite' }} />
        <div className="absolute left-[30%] bottom-[15%] h-[350px] w-[350px] rounded-full"
          style={{ background: 'radial-gradient(circle, oklch(0.85 0 0 / 4%), transparent 65%)', filter: 'blur(70px)', animation: 'float-orb-3 24s ease-in-out infinite' }} />
      </div>

      <div className="glass-card rounded-3xl px-8 py-10 w-full max-w-sm relative z-10">
        <div className="flex flex-col items-center gap-4 mb-8">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-xs tracking-wider text-muted-foreground">
              {step === 'invite' ? 'Enter your invite code' : 'Set up your login method'}
            </p>
          </div>
        </div>
        ...
        {step === 'invite' && (
          <form onSubmit={handleClaimInvite} className="flex flex-col gap-3">
            <input
              ...
              className="glass-card rounded-xl px-4 py-3 text-sm tracking-widest text-foreground placeholder:text-muted-foreground/40 bg-transparent border-0 outline-none focus:ring-1 focus:ring-white/10 w-full"
              autoComplete="off"
            ...
            <button
              ...
              className="glass-button glass-button-default rounded-xl px-6 py-3 text-sm tracking-widest disabled:opacity-50 flex items-center justify-center gap-2"
            >
        ...
        {step === 'choose' && (
          <div className="flex flex-col gap-3">
            <p className="text-xs tracking-widest text-muted-foreground text-center mb-2">
            ...
            <button
              ...
              disabled={isLoading}
              className="glass-button glass-button-glass rounded-xl px-6 py-3.5 text-sm tracking-wider flex items-center gap-3 disabled:opacity-50"
            >
              <KeyIcon />
            ...
            <button
              ...
              disabled={isLoading}
              className="glass-button rounded-xl px-6 py-3.5 text-sm tracking-wider flex items-center gap-3 disabled:opacity-50"
              style={{
                background: '#5865F2',
            ...
            {errorMsg && (
              <p className="text-xs tracking-wider text-center" style={{ color: 'oklch(0.7 0.19 22)' }}>
                {errorMsg}
              </p>
            )}
