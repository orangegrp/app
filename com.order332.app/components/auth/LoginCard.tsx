...
const OAUTH_ERROR_MESSAGES: Record<string, string> = {
  ...
  expired_registration:
    "Registration expired. Claim a new invite and try again.",
  account_disabled: "This account is disabled.",
  auth_failed: "Discord sign-in failed. Please try again.",
  invalid_token: "Session token was invalid. Please sign in again.",
  method_disabled:
    "Discord sign-in is turned off for this account. Use another method or enable it in Settings.",
  ...
  const handlePasskeyLogin = async () => {
    ...
    try {
      ...
      if (!challengeRes.ok) {
        ...
        setFatalError(
          body.error ?? "Failed to start passkey login. Please try again."
        )
        return
      }
      ...
      if (!verifyRes.ok) {
        ...
        setFatalError(
          body.error ?? "Passkey authentication failed. Please try again."
        )
        return
      }
      const { accessToken } = (await verifyRes.json()) as {
        accessToken: string
      }
      ...
      {fatalError ? (
        <div className="flex flex-col items-center gap-5 py-4 text-center">
          ...
          <button
            ...
            className="glass-button glass-button-glass w-full rounded-xl px-6 py-3 text-sm tracking-widest"
          >
            Back
          </button>
        </div>
        ...
        <>
          ...
          <div className="mb-6 grid grid-cols-4 gap-1 rounded-xl border border-white/10 bg-white/[0.06] p-1">
            {tabs.map((tab) => (
              <button
                ...
                className={[
                  "flex flex-col items-center gap-1 rounded-2xl px-1 py-2 text-xs tracking-wider transition-all",
                  activeTab === tab.id
          ...
          {activeTab === "passkey" && (
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="glass-button glass-button-glass flex w-full items-center justify-center gap-2 rounded-xl px-6 py-3.5 text-sm tracking-wider disabled:opacity-50"
              >
          ...
          {activeTab === "discord" && (
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className="glass-button flex w-full items-center justify-center gap-3 rounded-xl px-6 py-3.5 text-sm tracking-widest text-white"
                style={{
                  background: "#5865F2",
          ...
          {activeTab === "magic" && (
            <MagicLinkForm onFatalError={setFatalError} />
          )}
        ...
        <div className="mt-4 border-t border-white/5 pt-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="glass-button glass-button-ghost flex-1 rounded-lg px-3 py-2 text-xs tracking-widest text-muted-foreground hover:text-foreground"
            >
              Member
            </button>
            <button
