...
const RESIZE_HANDLES = [
  {
    id: "tl",
    cls: "absolute top-0 -translate-y-1/2 -left-1.5 cursor-nw-resize",
    dx: -1 as const,
    dy: -1 as const,
  },
  {
    id: "tm",
    cls: "absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 cursor-n-resize",
    dx: 0 as const,
    dy: -1 as const,
  },
  {
    id: "tr",
    cls: "absolute top-0 -translate-y-1/2 -right-1.5 cursor-ne-resize",
    dx: 1 as const,
    dy: -1 as const,
  },
  {
    id: "ml",
    cls: "absolute top-1/2 -translate-y-1/2 -left-1.5 cursor-w-resize",
    dx: -1 as const,
    dy: 0 as const,
  },
  {
    id: "mr",
    cls: "absolute top-1/2 -translate-y-1/2 -right-1.5 cursor-e-resize",
    dx: 1 as const,
    dy: 0 as const,
  },
  {
    id: "bl",
    cls: "absolute bottom-0 translate-y-1/2 -left-1.5 cursor-sw-resize",
    dx: -1 as const,
    dy: 1 as const,
  },
  {
    id: "bm",
    cls: "absolute bottom-0 translate-y-1/2 left-1/2 -translate-x-1/2 cursor-s-resize",
    dx: 0 as const,
    dy: 1 as const,
  },
  {
    id: "br",
    cls: "absolute bottom-0 translate-y-1/2 -right-1.5 cursor-se-resize",
    dx: 1 as const,
    dy: 1 as const,
  },
] as const
...
function ResizableImageView({
  ...
  const startResize = (e: React.MouseEvent, dx: -1 | 0 | 1, dy: -1 | 0 | 1) => {
    ...
    const onMouseMove = (ev: MouseEvent) => {
      const delta =
        dx !== 0 ? (ev.clientX - startX) * dx : (ev.clientY - startY) * dy
      updateAttributes({
        width: Math.min(
          2000,
          Math.max(MIN_IMAGE_WIDTH, Math.round(startWidth + delta))
        ),
      })
    }
    const onMouseUp = () => {
  ...
  const displayWidth =
    imgWidth && imgWidth < MIN_IMAGE_WIDTH ? MIN_IMAGE_WIDTH : imgWidth
        ...
        {selected && !confirming && (
          <>
            ...
            <button
              ...
              title="Delete image"
              className="absolute top-1/2 left-1/2 flex h-7 w-7 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-black/50 text-white/80 transition-colors hover:bg-black/80 hover:text-white"
            >
              <Trash2 size={13} />
            </button>
          </>
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-white/10 bg-popover p-4 shadow-xl">
              ...
              <div className="flex flex-col gap-1.5">
                <button
                  ...
                  className="rounded-md bg-red-500/80 px-3 py-1.5 text-left text-xs text-white transition-colors hover:bg-red-500"
                >
                ...
                <button
                  ...
                  onMouseDown={(e) => e.preventDefault()}
                  onClick={() => deleteNode()}
                  className="rounded-md border border-white/10 bg-white/5 px-3 py-1.5 text-left text-xs text-foreground transition-colors hover:bg-white/10"
                >
                ...
                <button
                  ...
                  className="rounded-md px-3 py-1.5 text-left text-xs text-muted-foreground transition-colors hover:text-foreground"
                >
                  Cancel
                </button>
              </div>
    ...
    return {
      markdown: {
        serialize(
          ...
          if (node.attrs.width) {
            state.write(
              `<img src="${src}" alt="${alt}" width="${node.attrs.width as number}" />`
            )
          } else {
...
export const VisualEditor = forwardRef<VisualEditorHandle, Props>(
      ...
      () => ({
        ...
        getSelectionMeta: (): BlogEditorSelectionMeta | null => {
          ...
          try {
            ...
            const tempDoc = ed.state.schema.topNodeType.createAndFill(
              null,
              slice.content
            )
            if (tempDoc) {
        ...
        replaceSelection: (text: string, from?: number, to?: number) => {
          ...
          try {
            const html = (
              ed.storage.markdown as { parser: { parse(s: string): string } }
            ).parser.parse(text)
    ...
    return (
      <div className="flex h-full flex-col">
        ...
        {hasJsxComponents(value) && (
          <div className="flex shrink-0 items-center gap-2 border-b border-yellow-500/20 bg-yellow-500/10 px-4 py-2 text-xs text-yellow-300/80">
            <AlertCircle size={12} />
            Visual mode may strip custom MDX JSX components. Switch to Raw mode
            to use JSX.
          </div>
        )}
          ...
          <DialogContent showCloseButton={false} className="max-w-sm">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" &&
                    !linkHasSelection &&
                    (e.preventDefault(), setLinkUrl(linkUrl))
                  }
                  placeholder="https://"
                  autoFocus
                  className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:ring-1 focus:ring-white/20 focus:outline-none"
                />
              </div>
              {linkHasSelection ? (
                <p className="text-xs text-muted-foreground">
                  Linking selected text:{" "}
                  <span className="font-medium text-foreground">{`"${linkText}"`}</span>
                </p>
              ) : (
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs tracking-wide text-muted-foreground">
                  ...
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && (e.preventDefault(), confirmLink())
                    }
            ...
            <DialogFooter>
              <button
                ...
                className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-xs tracking-wide text-muted-foreground transition-colors hover:text-foreground"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={confirmLink}
                className="rounded-lg bg-white/90 px-4 py-2 text-xs font-medium tracking-wide text-black transition-colors hover:bg-white"
              >
          ...
          <DialogContent showCloseButton={false} className="max-w-sm">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex gap-3">
                <div className="flex flex-1 flex-col gap-1.5">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-foreground focus:ring-1 focus:ring-white/20 focus:outline-none"
                  />
                </div>
                <div className="flex flex-1 flex-col gap-1.5">
                  <label className="text-xs tracking-wide text-muted-foreground">
              ...
              <p className="text-xs leading-relaxed text-muted-foreground/60">
                Table dimensions cannot be changed in the visual editor after
                creation. Use the raw editor to add or remove rows and columns.
              </p>
            </div>
            <DialogFooter>
              <button
