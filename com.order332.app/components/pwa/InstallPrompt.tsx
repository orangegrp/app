...
import { getSuppressInstallPromptSync, useSettingsStore } from '@/lib/settings-store'
...
function detectMode(): Mode {
  ...
  const isIOS = /iPad|iPhone|iPod/.test(ua) && !(ua.includes('Chrome') || ua.includes('CriOS'))
...
export function InstallPrompt() {
  ...
  const setSuppressInstallPrompt = useSettingsStore((s) => s.setSuppressInstallPrompt)
  ...
  return (
    ...
    >
      <div
        className="glass-card rounded-2xl w-full max-w-sm p-6 flex flex-col gap-5"
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm tracking-wider text-muted-foreground mt-1">
              ...
              {mode === 'ios26' ? (
                <>
                  <li className="flex items-start gap-3 text-sm tracking-wider text-muted-foreground">
                    ...
                    <span>
                      Tap the{' '}
                      <ShareIcon className="inline-block mx-1 align-text-top" />
                      {' '}Share button in the <strong className="text-foreground font-normal">bottom toolbar</strong>
                    </span>
                  </li>
                  <li className="flex items-start gap-3 text-sm tracking-wider text-muted-foreground">
                    <StepNum>2</StepNum>
                    <span>Tap <strong className="text-foreground font-normal">Add to Home Screen</strong></span>
                  </li>
                  <li className="flex items-start gap-3 text-sm tracking-wider text-muted-foreground">
                    <StepNum>3</StepNum>
                    <span>Tap <strong className="text-foreground font-normal">Add</strong> to confirm</span>
                  </li>
                </>
              ) : (
                <>
                  ...
                  <li className="flex items-start gap-3 text-sm tracking-wider text-muted-foreground">
                    <StepNum>2</StepNum>
                    <span>Scroll down and tap <strong className="text-foreground font-normal">Add to Home Screen</strong></span>
                  </li>
                  <li className="flex items-start gap-3 text-sm tracking-wider text-muted-foreground">
                    <StepNum>3</StepNum>
                    <span>Tap <strong className="text-foreground font-normal">Add</strong> to confirm</span>
...
function DontAskAgainRow({
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl px-1 py-0.5">
      ...
      <Label
        htmlFor={id}
        className="text-sm tracking-wider text-muted-foreground font-normal cursor-pointer leading-snug"
      >
        Don&apos;t ask again
      </Label>
    </div>
...
function StepNum({ children }: { children: React.ReactNode }) {
  return (
    <span
      className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-medium mt-0.5"
