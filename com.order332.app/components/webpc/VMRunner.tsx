...
export function VMRunner({
  ...
  const [lockNotif, setLockNotif] = useState<"captured" | "released" | null>(
    null
  )
  ...
  useEffect(() => {
    ...
    function translateEvent(e: MouseEvent) {
      ...
      const lockEl =
        ...
        (document as WDoc).webkitPointerLockElement
      ...
      if (e.type === "mousemove") {
        ...
        virtualMousePos.current = {
          x: Math.max(
            0,
            Math.min(rect.width, virtualMousePos.current.x + e.movementX)
          ),
          y: Math.max(
            0,
            Math.min(rect.height, virtualMousePos.current.y + e.movementY)
          ),
        }
    ...
    function onLockChange() {
      ...
      if (locked) {
        ...
        setTimeout(() => {
          pointerLockCooldownRef.current = false
        }, 1500)
      }
    ...
    return () => {
      EVENTS.forEach((ev) =>
        canvas.removeEventListener(ev, translateEvent, true)
      )
  ...
  function handleCanvasClick(e: React.MouseEvent) {
    ...
    const lockEl =
      document.pointerLockElement ??
      (document as { webkitPointerLockElement?: Element })
    ...
    if (!lockEl && !pointerLockCooldownRef.current) {
      ...
      try {
        ...
        /* SecurityError: browser cooldown — safe to ignore */
      }
    }
  }
  ...
  function onDragStart(e: React.MouseEvent | React.TouchEvent) {
    ...
    function onMove(ev: MouseEvent | TouchEvent) {
      ...
      setTermHeight(
        Math.max(
          TERMINAL_MIN_HEIGHT,
          Math.min(TERMINAL_MAX_HEIGHT, dragState.current.startH + delta)
        )
      )
    }
    function onUp() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        if (cancelled) return
        const overlayDevice = await CX.OverlayDevice.create(
          baseDevice,
          idbDevice
        )
        if (cancelled) return
        ...
        if (spec.needsDisplay) {
          ...
          onReady?.({
            ...
            switchTTY: (idx: number) =>
              ttyEventListener(new CustomEvent("vt", { detail: idx })),
          })
  ...
  if (vmStatus === "error") {
    return (
      ...
      >
        <div className="glass-card flex max-w-md flex-col items-center gap-4 rounded-2xl px-8 py-7 text-center">
          <svg
          ...
          <div>
            ...
            <p className="mb-3 text-sm leading-relaxed tracking-wider text-muted-foreground">
              {errorMsg ||
                "An unknown error occurred while starting the virtual machine."}
            </p>
            <p className="text-xs tracking-wider text-muted-foreground opacity-50">
      ...
      <div className="flex h-full flex-col overflow-hidden bg-[#0a0a0a]">
        ...
        >
          ...
          {vmStatus === "ready" && !guiActive && (
            ...
            >
              <div className="glass-card mx-4 flex w-full max-w-xs flex-col items-center gap-4 rounded-2xl px-8 py-7 text-center">
                <Spinner size="md" clockwise={true} />
                <div>
                  <p className="card-label mb-1.5">Graphical Environment</p>
                  <p className="max-w-[260px] truncate text-xs leading-relaxed tracking-wider opacity-70">
                    {bootLine || "Booting"}
        ...
        {terminalOpen && (
          <>
            ...
            >
              <div className="absolute inset-x-0 top-1/2 mx-auto h-[3px] w-12 -translate-y-1/2 rounded-full bg-white/10 transition-colors duration-150 group-hover:bg-white/25" />
            </div>
            <div
              className="flex shrink-0 items-center justify-between border-t border-white/5 px-3 py-1"
              style={{ background: "oklch(0.12 0 0)" }}
            >
              <span className="text-xs tracking-widest text-muted-foreground opacity-60">
                tty<span className="blink-cursor">_</span>
              </span>
              <span className="text-xs tracking-wider text-muted-foreground opacity-40 select-none">
                drag to resize
              </span>
            </div>
            <div
