...
app.get("/version", (c) =>
  c.json({ version: process.env.NEXT_PUBLIC_APP_VERSION ?? "dev" })
)
...
// POST   /content/items/video/upload-url   — create Mux direct upload URL
// POST   /content/items/video/complete     — register uploaded Mux video item
// POST   /content/items/video/source       — signed HLS source for playback
// POST   /content/items/video/download-url — temporary master download URL
...
app.get("/cron/cleanup", async (c) => {
  ...
  if (
    ...
    !safeCompare(authHeader.slice(7), secret)
  ) {
    return c.json({ error: "unauthorized" }, 401)
  }
...
app.post(
  "/auth/cleanup",
  requireAuth,
  requirePermission(PERMISSIONS.ADMIN_SYSTEM_CLEANUP),
  async (c) => {
    await db.cleanupExpiredRecords()
    return c.json({ ok: true })
  }
