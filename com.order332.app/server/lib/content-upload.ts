...
export const CONTENT_SIZE_LIMITS: Record<ContentItemType, number> = {
  ...
  audio: 3 * 1024 * 1024, // 3 MB
  pdf: 3 * 1024 * 1024, // 3 MB
  download: 3 * 1024 * 1024, // 3 MB
}
...
export function inferItemType(mimeType: string): ContentItemType | null {
  for (const [type, mimes] of Object.entries(CONTENT_ALLOWED_TYPES) as [
    ContentItemType,
    Set<string>,
  ][]) {
...
export async function uploadContentItemBuffer({
  ...
  contentType,
  filenameHint = "file",
}: UploadContentItemParams): Promise<{
  storageKey: string
  publicUrl: string
}> {
  try {
  ...
  const safeBase =
    filenameHint.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 80) || "file"
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from(CONTENT_LIBRARY_BUCKET).getPublicUrl(key)
